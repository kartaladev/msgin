...
func (c *consumer[T]) safePoll(ctx context.Context, max int) (rows []msgin.Delivery, err error) {
	defer func() {
		if r := recover(); r != nil {
...
func (c *consumer[T]) clampExcess(settleCtx context.Context, rows []msgin.Delivery, held int) []msgin.Delivery {
...
// error: min(maxPollErrorBackoff, pollInterval * 2^(n-1)). Reset to zero on the
// first successful poll by the caller.
//
