...
func TestConsume(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.assert(t, endpoint.Consume(tc.svc(&seen))(next).Handle(t.Context(), msgin.New[any](tc.input)), seen, forwarded)
		})
	}
}
...
func TestActivator_NilFn(t *testing.T) {
	...
	assert.ErrorIs(t, endpoint.Activate[int, int](nil)(next).Handle(t.Context(), msgin.New[any](1)), msgin.ErrNilFunc)
	assert.ErrorIs(t, endpoint.Consume[int](nil)(next).Handle(t.Context(), msgin.New[any](1)), msgin.ErrNilFunc)
}
