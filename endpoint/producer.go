...
type Producer[T any] interface {
	Send(ctx context.Context, msg msgin.Message[T]) error
	...
	// now+delay.
	SendAfter(ctx context.Context, msg msgin.Message[T], delay time.Duration) error
	...
	SendAt(ctx context.Context, msg msgin.Message[T], t time.Time) error
}
...
func WithProducerCodec[T any](c msgin.PayloadCodec[T]) ProducerOption[T] {
...
func WithProducerRetry[T any](p msgin.RetryPolicy) ProducerOption[T] {
...
func WithProducerHooks[T any](h msgin.Hooks) ProducerOption[T] {
...
func NewProducer[T any](out msgin.OutboundAdapter, opts ...ProducerOption[T]) (Producer[T], error) {
	...
	if cfg.retrySet {
		...
		// strategy's actual output. Evaluating retryDelay(cfg.retry, 1) here would
...
func (p *producer[T]) box(msg msgin.Message[T]) (msgin.Message[any], error) {
	if p.liveValue {
		return msgin.NewMessage[any](msg.Payload(), msg.Headers()), nil
	}
	b, err := p.codec.Encode(msg.Payload())
	if err != nil {
		return msgin.Message[any]{}, fmt.Errorf("msgin: producer encode failed: %w", err)
	}
	return msgin.NewMessage[any](any(b), msg.Headers()), nil
}
...
func (p *producer[T]) Send(ctx context.Context, msg msgin.Message[T]) error {
	boxed, err := p.box(msg)
	if err != nil {
		return err
	}
...
// attempt is 1-based, matching retryDelay.s contract. deadline bounds
// the cumulative wall-clock spent retrying (WithProducerRetryBudget).
func (p *producer[T]) sendRetrying(ctx context.Context, boxed msgin.Message[any]) error {
	...
	for attempt := 1; ; attempt++ {
		...
		if p.clock.Now().Add(wait).After(deadline) {
			...
			return p.deadLetter(ctx, boxed, fmt.Errorf("%w: %w", msgin.ErrRetryBudgetExhausted, err))
		}
...
func (p *producer[T]) nextDelay(attempt int, err error) time.Duration {
	d := retryDelay(p.retry, attempt)
	if server, ok := msgin.RetryAfterOf(err); ok {
...
func (p *producer[T]) deadLetter(ctx context.Context, boxed msgin.Message[any], cause error) error {
	...
	if dlErr := p.safeDeadLetter(dlCtx, boxed); dlErr != nil {
		...
		out = fmt.Errorf("%w: %w", msgin.ErrDeadLettered, cause)
	}
...
func (p *producer[T]) safeDeadLetter(ctx context.Context, msg msgin.Message[any]) (err error) {
	defer func() {
		if r := recover(); r != nil {
...
func (p *producer[T]) fireHook(h func(context.Context, msgin.Message[any], error), ctx context.Context, msg msgin.Message[any], err error) {
	if h == nil {
		return
	}
	defer func() {
...
func (p *producer[T]) SendAfter(ctx context.Context, msg msgin.Message[T], delay time.Duration) error {
	sched, ok := p.out.(msgin.ScheduledSender)
	if !ok {
		return msgin.ErrScheduledSendUnsupported
	}
	if delay < 0 {
		delay = 0
	}
...
func (p *producer[T]) SendAt(ctx context.Context, msg msgin.Message[T], t time.Time) error {
...
func resolveCodec[T any](adapter any, codec msgin.PayloadCodec[T], codecSet bool) (msgin.PayloadCodec[T], bool, error) {
	live := isLiveValue(adapter)
	if live && codecSet {
		return nil, live, msgin.ErrUnexpectedCodec
	}
	if !live && codec == nil {
		codec = msgin.JSONPayloadCodec[T]{}
	}
