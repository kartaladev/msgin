...
func runConsumer(t *testing.T, c endpoint.Consumer[order]) (stop func()) {
	t.Helper()
...
func TestConsumer_StreamingDeliversToHandler(t *testing.T) {
	...
	p, err := endpoint.NewProducer[order](b)
	...
	c, err := endpoint.NewConsumer[order](b, h) // memory ⇒ no codec (live value)
	require.NoError(t, err)
...
func TestNewConsumer_RetryPolicyValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{"finite policy without dead-letter is rejected at construction",
			[]endpoint.ConsumerOption[order]{endpoint.WithRetryPolicy[order](msgin.RetryPolicy{MaxAttempts: 3})},
			func(t *testing.T, err error) { assert.ErrorIs(t, err, msgin.ErrNoDeadLetter) }},
		{"negative MaxAttempts is rejected",
			[]endpoint.ConsumerOption[order]{endpoint.WithRetryPolicy[order](msgin.RetryPolicy{MaxAttempts: -1})},
		...
		{"valid reliability options construct cleanly",
			[]endpoint.ConsumerOption[order]{
				...
				endpoint.WithLogger[order](slog.New(slog.NewTextHandler(io.Discard, nil))),
				endpoint.WithHooks[order](msgin.Hooks{}),
				endpoint.WithShutdownTimeout[order](time.Second),
				endpoint.WithConsumerClock[order](clockwork.NewFakeClock()),
				endpoint.WithInvalidMessageSink[order](memory.New()),
			},
			func(t *testing.T, err error) { assert.NoError(t, err) }},
	}
	for _, tc := range tests {
...
func TestConsumer_DecodeAndDispatchSettle(t *testing.T) {
	...
	tests := []struct {
		name       string
		payload    any
		newHandler func(rec *handlerRecorder) endpoint.Handler[order]
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			c, err := endpoint.NewConsumer[order](src, tc.newHandler(hrec))
			require.NoError(t, err)
...
func TestConsumer_Settlement_PermanentAndTransient(t *testing.T) {
	...
	tests := []struct {
		name    string
		handler endpoint.Handler[orderPayload]
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			opts := []endpoint.ConsumerOption[orderPayload]{}
			if tc.sink != nil {
				opts = append(opts, endpoint.WithInvalidMessageSink[orderPayload](tc.sink))
			}
			c, err := endpoint.NewConsumer[orderPayload](src, tc.handler, opts...)
			require.NoError(t, err)
...
func TestConsumer_PanickingHook_IsContainedAndFlowContinues(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			c, err := endpoint.NewConsumer[order](src, h,
				endpoint.WithInvalidMessageSink[order](&recordingSink{}),
				endpoint.WithHooks[order](panicHooks))
			require.NoError(t, err)
...
func TestConsumer_AckFailure_DoesNotEvictTracker(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](src, h,
		endpoint.WithRetryPolicy[order](msgin.RetryPolicy{MaxAttempts: 2, DeadLetter: dlq}))
	require.NoError(t, err)
...
func TestConsumer_DecodeFailure_DivertsToInvalidNotRetried(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](src, h, endpoint.WithInvalidMessageSink[order](sink))
	require.NoError(t, err)
...
func TestConsumer_Settlement_ConcurrentWorkers(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](src, h,
		endpoint.WithConcurrency[order](4),
		endpoint.WithInvalidMessageSink[order](sink),
		endpoint.WithHooks[order](r.hooks()))
	require.NoError(t, err)
...
func TestConsumer_Hooks_Fire(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			r := newHookRec()
			src := &scriptedSource{deliveries: []msgin.Delivery{newSettleDelivery(order{ID: "o"}, "m1", st)}}
			c, err := endpoint.NewConsumer[order](src, tc.handler,
				endpoint.WithInvalidMessageSink[order](&recordingSink{}),
				endpoint.WithHooks[order](r.hooks()))
			require.NoError(t, err)
...
func TestConsumer_DeadLetterAfterMaxAttempts(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](src, h,
		...
		endpoint.WithRetryPolicy[order](msgin.RetryPolicy{
			MaxAttempts: 3,
			DeadLetter:  dlq,
			Backoff:     resilience.ExponentialBackoff{}, // zero => immediate; keeps the test fast
		}))
	require.NoError(t, err)
...
func TestConsumer_RetryBackoffDelay_ReachesNack(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](src, h, endpoint.WithRetryPolicy[order](msgin.RetryPolicy{
		MaxAttempts: 0,                                                           // retry forever
		Backoff:     resilience.ExponentialBackoff{Initial: initial, Mult: mult}, // RandomizationFactor 0 => exact
	}))
	require.NoError(t, err)
...
func TestConsumer_NativeDeadLetter_FallsThroughToRedelivery(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](src, h,
		endpoint.WithRetryPolicy[order](msgin.RetryPolicy{MaxAttempts: 1, DeadLetter: dlq}))
	require.NoError(t, err)
...
func TestConsumer_Shutdown_TransientNackDuringDrain(t *testing.T) {
	tests := []struct {
		name string
		opts func(clk *clockwork.FakeClock) []endpoint.ConsumerOption[order]
	}{
		{"default shutdown timeout (finite, not infinite)", func(clk *clockwork.FakeClock) []endpoint.ConsumerOption[order] {
			return []endpoint.ConsumerOption[order]{endpoint.WithConsumerClock[order](clk)}
		}},
		{"explicit small shutdown timeout", func(clk *clockwork.FakeClock) []endpoint.ConsumerOption[order] {
			return []endpoint.ConsumerOption[order]{
				endpoint.WithConsumerClock[order](clk),
				endpoint.WithShutdownTimeout[order](50 * time.Millisecond),
			}
		}},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			c, err := endpoint.NewConsumer[order](b, h, tc.opts(clk)...)
			require.NoError(t, err)
...
func TestConsumer_ShutdownDeadlineExpiry_NacksInflightAndBuffered(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		c, err := endpoint.NewConsumer[order](src, h,
			endpoint.WithConsumerClock[order](clk),
			endpoint.WithShutdownTimeout[order](5*time.Second))
		require.NoError(t, err)
...
func TestConsumer_ShutdownDeadlineExpiry_DrainsWorkerChBufferResident(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		c, err := endpoint.NewConsumer[order](src, h,
			endpoint.WithConsumerClock[order](clk),
			endpoint.WithMaxInFlight[order](3), // n >= 2: room for "second"+"third" to sit IN workerCh's buffer
			endpoint.WithShutdownTimeout[order](5*time.Second))
		require.NoError(t, err)
...
func TestConsumer_GracefulDrain_ProcessesBuffered(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](b, h) // shutdownTimeout 0 => finite default drain
	require.NoError(t, err)
...
func TestConsumer_MaxInFlight_BoundsClaimedUnsettled(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		c, err := endpoint.NewConsumer[order](src, h,
			...
			endpoint.WithConsumerClock[order](clockwork.NewFakeClock()),
			endpoint.WithConcurrency[order](burst), // many workers…
			endpoint.WithMaxInFlight[order](n))     // …but credit caps in-flight at n
		require.NoError(t, err)
...
func TestConsumer_MaxInFlight_NackReleasesSlot_NoDeadlock(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](b, h, endpoint.WithMaxInFlight[order](1))
	require.NoError(t, err)
...
func TestConsumer_MaxInFlight_ShutdownWithIngestHeldSurplus_NoLeak(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](b, h,
		endpoint.WithConsumerClock[order](clk),
		endpoint.WithMaxInFlight[order](1))
	require.NoError(t, err)
...
func TestConsumer_RateLimit_GatesIngress(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](b, h, endpoint.WithRateLimit[order](rl))
	require.NoError(t, err)
...
func TestConsumer_HandlerTimeout_CancelsStuckHandlerAndRetries(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](src, h,
		endpoint.WithConsumerClock[order](clk),
		endpoint.WithHandlerTimeout[order](2*time.Second))
	require.NoError(t, err)
...
func TestConsumer_CircuitBreaker_GatesDispatchAndRecovers(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](broker, h,
		endpoint.WithConsumerClock[order](clk),
		endpoint.WithCircuitBreaker[order](b))
	require.NoError(t, err)
...
func TestConsumer_Overflow_UnknownPolicy_BlocksNeverDrops(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		c, err := endpoint.NewConsumer[order](b, h,
			endpoint.WithConsumerClock[order](clockwork.NewFakeClock()), // I3
			endpoint.WithMaxInFlight[order](1),
			endpoint.WithOverflow[order](msgin.OverflowPolicy(99)), // out of range → must behave as Block
			endpoint.WithHooks[order](hooks))
		require.NoError(t, err)
...
func TestConsumer_AttemptTracker_TTLSweepResetsIdleIds(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		c, err := endpoint.NewConsumer[order](src, h,
			endpoint.WithConsumerClock[order](clk),
			endpoint.WithRetryPolicy[order](msgin.RetryPolicy{
				Backoff: resilience.ExponentialBackoff{Initial: initial, Mult: 2.0}, // RandomizationFactor 0 => exact
			}))
		require.NoError(t, err)
...
func TestConsumer_AttemptTracker_TTLSweepKeepsRefreshedId(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		c, err := endpoint.NewConsumer[order](src, h,
			endpoint.WithConsumerClock[order](clk),
			endpoint.WithRetryPolicy[order](msgin.RetryPolicy{
				Backoff: resilience.ExponentialBackoff{Initial: initial, Mult: mult},
			}))
		require.NoError(t, err)
...
func TestConsumer_MaxPayloadBytes_CapsWirePayload(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			c, err := endpoint.NewConsumer[order](src, h,
				endpoint.WithMaxPayloadBytes[order](tc.max),
				endpoint.WithInvalidMessageSink[order](sink),
				endpoint.WithHooks[order](hooks.hooks()))
			require.NoError(t, err)
...
func TestConsumer_WithAttemptTTL_UsesConfiguredTTL(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		c, err := endpoint.NewConsumer[order](src, h,
			endpoint.WithConsumerClock[order](clk),
			endpoint.WithAttemptTTL[order](time.Minute), // << far below the 5m default
			endpoint.WithRetryPolicy[order](msgin.RetryPolicy{
				Backoff: resilience.ExponentialBackoff{Initial: initial, Mult: 2.0},
			}))
		require.NoError(t, err)
...
func TestConsumer_SafeDecode_CodecPanicRoutesToInvalidWithErrPayloadDecode(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](src, h,
		endpoint.WithConsumerCodec[order](panicDecodeCodec{}),
		endpoint.WithHooks[order](hooks),
		endpoint.WithLogger[order](slog.New(slog.NewTextHandler(buf, nil))))
	require.NoError(t, err)
...
func TestConsumer_SafeSend_SinkPanicIsRoutedLikeASendError(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](src, h,
		endpoint.WithInvalidMessageSink[order](panicSendSink{}),
		endpoint.WithLogger[order](slog.New(slog.NewTextHandler(buf, nil))))
	require.NoError(t, err)
...
func TestConsumer_SafeSend_NilSinkPassthroughUnaffected(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](src, h) // no WithInvalidMessageSink → nil sink
	require.NoError(t, err)
