...
func TestPoller_NeverOverPulls(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		c, err := endpoint.NewConsumer[order](src, h,
			...
			endpoint.WithConcurrency[order](total), // many workers…
			endpoint.WithMaxInFlight[order](n),     // …but credit caps in-flight at n
			endpoint.WithPollMaxBatch[order](10))   // batch > n, so credit (not batch) is the cap
		require.NoError(t, err)
...
func TestPoller_ClampsExcessDeliveries(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](src, h,
		...
		endpoint.WithConsumerClock[order](clk),
		endpoint.WithMaxInFlight[order](2),
		endpoint.WithPollMaxBatch[order](2))
	require.NoError(t, err)
...
func TestPoller_ShutdownDrainsInFlightAndBounded(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](src, h,
		...
		endpoint.WithShutdownTimeout[order](500*time.Millisecond))
	require.NoError(t, err)
...
func TestPoller_HandoffCtxDoneBoundedNoLeak(t *testing.T) {
	...
	c, err := endpoint.NewConsumer[order](src, h,
		endpoint.WithMaxInFlight[order](n),
		endpoint.WithPollMaxBatch[order](n),
		endpoint.WithConcurrency[order](1),
		endpoint.WithShutdownTimeout[order](500*time.Millisecond))
	require.NoError(t, err)
