...
// order is the shared JSON-codec fixture payload. It is declared in the root
// codec_test.go too; both copies are test-only doubles, so the duplication is
// deliberate — the two test binaries are independent.
type order struct {
	ID    string `json:"id"`
