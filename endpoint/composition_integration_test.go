...
func TestComposition_DrivesOffConsumer(t *testing.T) {
	...
	flow := msgin.Chain(
		...
		endpoint.Consume(func(_ context.Context, m msgin.Message[Invoice]) error { got <- m.Payload(); return nil }),
	)

	consumer, err := endpoint.NewConsumer[any](src, flow.Handle, endpoint.WithShutdownTimeout[any](2*time.Second))
