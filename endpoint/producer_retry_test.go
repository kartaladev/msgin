...
func runSend(t *testing.T, p endpoint.Producer[[]byte], clock *clockwork.FakeClock, ctx context.Context) *retryHarness {
	t.Helper()
...
func TestProducerRetry(t *testing.T) {
		...
		},
		{
			...
			policy: func(dlq msgin.OutboundAdapter) msgin.RetryPolicy {
				return msgin.RetryPolicy{
					MaxAttempts: 3,
					Backoff:     resilience.ExponentialBackoff{Initial: time.Second, Mult: 2},
					DeadLetter:  dlq,
				}
			},
			assert: func(t *testing.T, out, dlq *scriptedOutbound, err error) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			opts := []endpoint.ProducerOption[[]byte]{
				endpoint.WithProducerClock[[]byte](clock),
			...
			if tt.policy != nil {
				opts = append(opts, endpoint.WithProducerRetry[[]byte](tt.policy(dlq)))
			}

			p, err := endpoint.NewProducer[[]byte](out, opts...)
			require.NoError(t, err)
...
func TestProducerPermanentBeatsRetryAfter(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			p, err := endpoint.NewProducer[[]byte](out,
				endpoint.WithProducerClock[[]byte](clock),
				endpoint.WithProducerRetry[[]byte](msgin.RetryPolicy{MaxAttempts: 5, DeadLetter: dlq}),
			)
			require.NoError(t, err)
...
func TestProducerDeadLetterFailure(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			p, err := endpoint.NewProducer[[]byte](out,
				endpoint.WithProducerClock[[]byte](clock),
				endpoint.WithProducerRetry[[]byte](msgin.RetryPolicy{MaxAttempts: 1, DeadLetter: tt.dlq}),
			)
			require.NoError(t, err)
...
func TestProducerDeadLetterDetachedContext(t *testing.T) {
	...
	p, err := endpoint.NewProducer[[]byte](out,
		endpoint.WithProducerClock[[]byte](clock),
		endpoint.WithProducerCodec[[]byte](msgin.BytesPayloadCodec{}),
		endpoint.WithProducerRetry[[]byte](msgin.RetryPolicy{MaxAttempts: 1, DeadLetter: dlq}),
	)
	require.NoError(t, err)
...
func TestProducerDeadLetterTimeout(t *testing.T) {
	...
	p, err := endpoint.NewProducer[[]byte](out,
		endpoint.WithProducerClock[[]byte](clock),
		endpoint.WithProducerDeadLetterTimeout[[]byte](time.Nanosecond),
		endpoint.WithProducerRetry[[]byte](msgin.RetryPolicy{MaxAttempts: 1, DeadLetter: dlq}),
	)
	require.NoError(t, err)
...
func TestProducerRetryContextCancel(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			policy := msgin.RetryPolicy{Backoff: resilience.ExponentialBackoff{Initial: time.Minute}}
			if tt.withDLQ {
			...
			p, err := endpoint.NewProducer[[]byte](out,
				endpoint.WithProducerClock[[]byte](clock),
				endpoint.WithProducerCodec[[]byte](msgin.BytesPayloadCodec{}),
				endpoint.WithProducerRetry[[]byte](policy), // MaxAttempts 0 = bounded by budget and ctx
			)
			require.NoError(t, err)
...
func TestProducerHooks(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			policy: func(msgin.OutboundAdapter) msgin.RetryPolicy {
				return msgin.RetryPolicy{Backoff: resilience.ExponentialBackoff{Initial: time.Second}}
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			p, err := endpoint.NewProducer[[]byte](out,
				endpoint.WithProducerClock[[]byte](clock),
				endpoint.WithProducerLogger[[]byte](slog.New(slog.NewTextHandler(&logs, nil))),
				endpoint.WithProducerRetry[[]byte](tt.policy(dlq)),
				endpoint.WithProducerHooks[[]byte](tt.hooks(&c)),
			)
			require.NoError(t, err)
...
func TestProducerRetryAfter(t *testing.T) {
	...
	tests := []struct {
		name     string
		first    error
		extra    []endpoint.ProducerOption[[]byte]
		...
		},
		{
			...
			extra: []endpoint.ProducerOption[[]byte]{
				endpoint.WithProducerRetryAfterCap[[]byte](2 * time.Minute),
				endpoint.WithProducerRetryBudget[[]byte](10 * time.Minute),
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			opts := append([]endpoint.ProducerOption[[]byte]{
				endpoint.WithProducerClock[[]byte](clock),
				endpoint.WithProducerRetry[[]byte](msgin.RetryPolicy{
					Backoff: resilience.ExponentialBackoff{Initial: time.Second, Mult: 2},
				}),
			}, tt.extra...)
...
func TestProducerRetryBudget(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			policy := msgin.RetryPolicy{Backoff: resilience.ExponentialBackoff{Initial: time.Second, Mult: 2}}
			if tt.withDLQ {
				policy.DeadLetter = dlq
			}
			...
			p, err := endpoint.NewProducer[[]byte](out,
				...
				endpoint.WithProducerRetryBudget[[]byte](6*time.Second),
			)
			require.NoError(t, err)
...
func TestNewProducerRetryValidation(t *testing.T) {
	...
	tests := []struct {
		name   string
		opts   []endpoint.ProducerOption[[]byte]
		assert func(t *testing.T, p endpoint.Producer[[]byte], err error)
	}{
		{
			name: "finite MaxAttempts without a DeadLetter is rejected",
			opts: []endpoint.ProducerOption[[]byte]{
				endpoint.WithProducerRetry[[]byte](msgin.RetryPolicy{MaxAttempts: 3}),
			},
			assert: func(t *testing.T, p endpoint.Producer[[]byte], err error) {
				t.Helper()
				assert.Nil(t, p)
		...
		},
		{
			...
			opts: []endpoint.ProducerOption[[]byte]{
				endpoint.WithProducerRetry[[]byte](msgin.RetryPolicy{MaxAttempts: -1, DeadLetter: sink}),
			},
			assert: func(t *testing.T, p endpoint.Producer[[]byte], err error) {
				t.Helper()
				assert.Nil(t, p)
		...
		},
		{
			...
			opts: []endpoint.ProducerOption[[]byte]{endpoint.WithProducerRetryAfterCap[[]byte](0)},
			assert: func(t *testing.T, p endpoint.Producer[[]byte], err error) {
				t.Helper()
				assert.Nil(t, p)
		...
		},
		{
			...
			opts: []endpoint.ProducerOption[[]byte]{endpoint.WithProducerRetryAfterCap[[]byte](-time.Second)},
			assert: func(t *testing.T, p endpoint.Producer[[]byte], err error) {
				t.Helper()
				assert.Nil(t, p)
				assert.ErrorIs(t, err, msgin.ErrInvalidRetryAfterCap)
		...
		},
		{
			...
			opts: []endpoint.ProducerOption[[]byte]{endpoint.WithProducerRetryBudget[[]byte](0)},
			assert: func(t *testing.T, p endpoint.Producer[[]byte], err error) {
				t.Helper()
				assert.Nil(t, p)
		...
		},
		{
			...
			opts: []endpoint.ProducerOption[[]byte]{endpoint.WithProducerRetryBudget[[]byte](-time.Second)},
			assert: func(t *testing.T, p endpoint.Producer[[]byte], err error) {
				t.Helper()
				assert.Nil(t, p)
				assert.ErrorIs(t, err, msgin.ErrInvalidRetryBudget)
		...
		},
		{
			...
			opts: []endpoint.ProducerOption[[]byte]{endpoint.WithProducerDeadLetterTimeout[[]byte](0)},
			assert: func(t *testing.T, p endpoint.Producer[[]byte], err error) {
				t.Helper()
				assert.Nil(t, p)
		...
		},
		{
			...
			opts: []endpoint.ProducerOption[[]byte]{endpoint.WithProducerDeadLetterTimeout[[]byte](-time.Second)},
			assert: func(t *testing.T, p endpoint.Producer[[]byte], err error) {
				t.Helper()
				assert.Nil(t, p)
				assert.ErrorIs(t, err, msgin.ErrInvalidDeadLetterTimeout)
		...
		},
		{
			...
			opts: []endpoint.ProducerOption[[]byte]{endpoint.WithProducerDeadLetterTimeout[[]byte](5 * time.Second)},
			assert: func(t *testing.T, p endpoint.Producer[[]byte], err error) {
				t.Helper()
				require.NoError(t, err)
				assert.NotNil(t, p)
		...
		},
		{
			...
			opts: []endpoint.ProducerOption[[]byte]{endpoint.WithProducerLogger[[]byte](nil)},
			assert: func(t *testing.T, p endpoint.Producer[[]byte], err error) {
				t.Helper()
				require.NoError(t, err)
				assert.NotNil(t, p)
		...
		},
		{
			...
			opts: []endpoint.ProducerOption[[]byte]{
				endpoint.WithProducerLogger[[]byte](slog.New(slog.NewTextHandler(io.Discard, nil))),
			},
			assert: func(t *testing.T, p endpoint.Producer[[]byte], err error) {
				t.Helper()
				require.NoError(t, err)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			p, err := endpoint.NewProducer[[]byte](sink, tt.opts...)
			tt.assert(t, p, err)
		})
	}
}
...
func TestProducerScheduledSendIsNotRetried(t *testing.T) {
	...
	clock := clockwork.NewFakeClock()
	p, err := endpoint.NewProducer[[]byte](sched,
		endpoint.WithProducerClock[[]byte](clock),
		endpoint.WithProducerRetry[[]byte](msgin.RetryPolicy{
			Backoff: resilience.ExponentialBackoff{Initial: time.Minute},
		}),
	)
	require.NoError(t, err)
