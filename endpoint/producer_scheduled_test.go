...
func TestProducer_SendAfter(t *testing.T) {
	...
	tests := []struct {
		name   string
		run    func(t *testing.T, p endpoint.Producer[string], sink *schedSink) error
		assert func(t *testing.T, err error, sink *schedSink)
	}{
		{
			name: "forwards the exact delay and encodes the payload",
			run: func(t *testing.T, p endpoint.Producer[string], _ *schedSink) error {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			sink := &schedSink{}
			p, err := endpoint.NewProducer[string](sink, endpoint.WithProducerClock[string](clockwork.NewFakeClockAt(epoch)))
			require.NoError(t, err)
...
func TestProducer_ScheduledSendUnsupported(t *testing.T) {
	p, err := endpoint.NewProducer[string](plainSink{})
	require.NoError(t, err)
...
func TestProducer_SendAfter_EncodeErrorPropagates(t *testing.T) {
	p, err := endpoint.NewProducer[order](&schedSink{}, endpoint.WithProducerCodec[order](failingCodec{}))
	require.NoError(t, err)
