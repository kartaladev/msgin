...
func ExampleConsumer_deadLetter() {
	...
	c, err := endpoint.NewConsumer[string](src, handler,
		endpoint.WithInvalidMessageSink[string](invalid),
...
func ExampleWithProducerRetry() {
	...
	p, err := endpoint.NewProducer[[]byte](flaky,
		endpoint.WithProducerCodec[[]byte](msgin.BytesPayloadCodec{}),
		endpoint.WithProducerRetry[[]byte](msgin.RetryPolicy{
			MaxAttempts: 3,
			Backoff:     resilience.ExponentialBackoff{Initial: time.Millisecond, Mult: 2},
			DeadLetter:  dlq,
		}),
	)
	if err != nil {
