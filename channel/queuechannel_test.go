...
func TestQueueChannel(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			assert: func(t *testing.T) {
				_, err := channel.NewQueueChannel(nil)
		...
		},
		{
			...
			assert: func(t *testing.T) {
				qc, err := channel.NewQueueChannel(newFakeStore(true))
		...
		},
		{
			...
			assert: func(t *testing.T) {
				...
				qc, err := channel.NewQueueChannel(fs)
				require.NoError(t, err)
		...
		},
		{
			...
			assert: func(t *testing.T) {
				live, _ := channel.NewQueueChannel(newFakeStore(true))
				wire, _ := channel.NewQueueChannel(newFakeStore(false))
		...
		},
		{
			...
			assert: func(t *testing.T) {
				plain, _ := channel.NewQueueChannel(newFakeStore(true)) // fakeStore is NOT NativeReliability
				require.False(t, plain.NativeRedelivery())
				require.False(t, plain.NativeDeadLetter())
				native, _ := channel.NewQueueChannel(nativeFake{newFakeStore(false)})
...
func ExampleQueueChannel() {
	...
	qc, _ := channel.NewQueueChannel(store)
