...
func TestExponentialBackoff_Delay(t *testing.T) {
	base := resilience.ExponentialBackoff{Initial: 100 * time.Millisecond, Max: time.Second, Mult: 2}
	...
	tests := []struct {
		...
		{"zero Initial yields zero", resilience.ExponentialBackoff{Initial: 0, Max: time.Second, Mult: 2}, 3,
			func(t *testing.T, d time.Duration) {
				assert.Equal(t, time.Duration(0), d)
			}},
		{"non-growing Mult stays at Initial", resilience.ExponentialBackoff{Initial: 50 * time.Millisecond, Max: time.Second, Mult: 0}, 4,
			func(t *testing.T, d time.Duration) {
				assert.Equal(t, 50*time.Millisecond, d)
			}},
		{"no Max means no cap", resilience.ExponentialBackoff{Initial: time.Second, Max: 0, Mult: 2}, 2,
			func(t *testing.T, d time.Duration) {
				assert.Equal(t, 4*time.Second, d)
			}},
		{"jitter stays within bounds", resilience.ExponentialBackoff{Initial: 100 * time.Millisecond, Max: time.Second, Mult: 2, RandomizationFactor: 0.5}, 1,
			func(t *testing.T, d time.Duration) {
		...
		{"overflow without Max returns Initial", resilience.ExponentialBackoff{Initial: time.Second, Max: 0, Mult: 2}, 2000,
			func(t *testing.T, d time.Duration) {
		...
		{"astronomic-but-finite growth uncapped falls back to Initial",
			resilience.ExponentialBackoff{Initial: time.Second, Max: 0, Mult: 2}, 100,
			func(t *testing.T, d time.Duration) {
		...
		{"jitter cannot push an uncapped backoff negative",
			resilience.ExponentialBackoff{Initial: time.Second, Max: 0, Mult: 2, RandomizationFactor: 0.5}, 33,
			func(t *testing.T, _ time.Duration) {
				...
				b := resilience.ExponentialBackoff{Initial: time.Second, Max: 0, Mult: 2, RandomizationFactor: 0.5}
				for i := range 200 {
					d := b.Delay(33)
					assert.Positive(t, d,
		...
		{"an infinite randomization factor cannot produce a negative delay",
			resilience.ExponentialBackoff{Initial: time.Second, Max: 0, Mult: 2, RandomizationFactor: math.Inf(1)}, 1,
			func(t *testing.T, d time.Duration) {
		...
		{"a NaN randomization factor skips jitter entirely",
			resilience.ExponentialBackoff{Initial: time.Second, Max: 0, Mult: 2, RandomizationFactor: math.NaN()}, 1,
			func(t *testing.T, d time.Duration) {
				// NaN fails the `RandomizationFactor > 0` test, so jitter is
				// never called and the computed backoff stands.
				assert.Equal(t, 2*time.Second, d)
			}},
		{"jitter never exceeds Max after cap", resilience.ExponentialBackoff{Initial: 100 * time.Millisecond, Max: time.Second, Mult: 2, RandomizationFactor: 0.5}, 10,
			func(t *testing.T, d time.Duration) {
...
func TestExponentialBackoff_JitterLowSideClamp(t *testing.T) {
	b := resilience.ExponentialBackoff{Initial: 100 * time.Millisecond, Max: time.Second, Mult: 2, RandomizationFactor: 2.0}
	for i := 0; i < 1000; i++ {
		d := b.Delay(i % 8)
...
func TestExponentialBackoff_SatisfiesInterface(t *testing.T) {
	var _ msgin.BackoffStrategy = resilience.ExponentialBackoff{}
}
