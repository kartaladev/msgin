...
// mysql.DDL(table) (adapter/database/sql/mysql) to obtain the reference
// CREATE TABLE (+ index) for your migration tool, or call the optional,
...
// postgres, its own module) and mysql.LeaseDialect() (adapter/database/sql/
// mysql, its own module). They are behavior-identical — same lease/claim
