...
func (sqliteGroupDialect) AddMember(ctx context.Context, q msginsql.Querier, table, groupKey, msgID string, seq int64, headers, payload []byte, maxMembers int, leaseTTL time.Duration) (msginsql.GroupRows, error) {
	if msgID == "" {
...
ON CONFLICT (group_key) DO NOTHING`, gt, nowMicros),
		...
		// The LEASE-LIVENESS flag rides out of the SAME statement that reads
		// created_at, at zero extra cost: it is what discriminates the
		// member-cap rejection's classification below (no live lease = nothing
		// drains this group = permanent; a live lease = a claim is in flight =
		// transient — Spec 017 §3.3.1 as corrected by §3.6a.1). It is
...
VALUES (?, ?, ?, ?, ?, NULL) ON CONFLICT (group_key, msg_id) DO NOTHING`, mt),
		...
		// Did THIS statement store the row? sqlite3_changes() counts rows the
		// most recent statement actually inserted, so 0 means the ON CONFLICT
		// (group_key, msg_id) DO NOTHING arm fired — and that arm names ONE
		// conflict target, so every other fault still errors out above. On this
		// engine 0 therefore proves durable presence, not failure.
		//
		...
		// defect): postgres reads it back with RETURNING, mysql and sqlite read
		// RowsAffected, and each dialect's choice is the one its OWN ClaimGroup
		// already relies on in this same file.
		...
		// no-op at the cap and equally above it (Spec 017 §3.6.1, review
		// finding R-11). A no-op stores nothing, so it cannot grow the group
		// past any bound; refusing it would terminally discard a redelivery the
		// SPI guarantees is idempotent, and which adapter/memory's GroupStore
		// answers from its dedup branch with (full snapshot, nil).
		//
		// The count is EVERY row for the key — live AND claimed — via the
		// shipped sqliteCountMembers: ClaimGroup stamps every live member, so a
		...
		// UNLIMITED, exactly like ClaimGroup's and ExpiredGroups' fetches:
		// a member set the caller ACTS ON must never be truncated (ADR 0033
		// D-AS). This call used to bound the fetch to maxMembers+1, which is
		// sound only while the group holds at most that many rows — false the
		// moment the cap is LOWERED beneath a group's existing size, by a
		// rolling deploy or by two instances configured differently. It then
		// returned a truncated snapshot that the release predicate reads as an
		// incomplete group, so the member is dead-lettered AND the complete
		// group never releases — the deadlock this snapshot exists to prevent
		// (review finding R-1). sqliteSelectMembers no longer takes a limit at
		// all, so the bound cannot be reintroduced at this call site; see its
		// godoc.
		//
		...
		if overflowErr != nil {
			...
			// inserted > 0, which is what makes removing msgID correct: a
