...
// DDL returns the reference CREATE TABLE statement for the lease/claim schema
// on MySQL, for callers to fold into their own migration tool. It validates
// table (ErrInvalidTableName on a bad identifier) before building the
...
// the idempotent-consumer dedup inbox on MySQL, for callers to fold into their
// own migration tool (ADR 0010 D10). It validates table (ErrInvalidTableName on
// a bad identifier) as the sole entry point, applying the same
// identifier-injection discipline as DDL (reference finding I1): there is no
// string-returning DDL method on the InboxDialect interface to bypass
// validation.
...
// identifiers. The retention index on processed_at is declared INLINE (MySQL
// has no CREATE INDEX IF NOT EXISTS), so the whole schema is a single statement.
