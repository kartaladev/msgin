...
// as a []byte payload: msghttp.DecodeRequest reads the raw body and never
// decodes it.
...
// be passed to NewInbound at all (the RED artifact is a compile failure, not a
// FAIL line).
...
// TestNewInboundTargetAcceptsEveryChannel is this package's half of the ADR
// 0028 capability test (Spec 014 §9.4 row 8): NewInbound's target parameter
...
// this file and adapter/http's carry the two HTTP ones.
//
// It is a REGRESSION FENCE around the widening, not a red-to-green proof:
// NewInbound already takes msgin.MessageChannel, so each subtest passes the
// first time it compiles. Its failure mode is a future NARROWING of that
// parameter, which would break the build here.
func TestNewInboundTargetAcceptsEveryChannel(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			tgt := tc.build(t)
			h, err := stdlib.NewInbound(tgt.ch)
			require.NoError(t, err)
