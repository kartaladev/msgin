...
**Active, pre-v1.** The module exists and is substantially implemented: the pattern core at the repo root plus `adapter/{memory,cron,http,http/stdlib,database/sql}`, delivered across 26 plans / 26 ADRs / 13 specs. **Nothing is tagged or released** — there are no consumers, so breaking API changes are still free. Read [`docs/HANDOVER.md`](docs/HANDOVER.md) first: it names the active increment and the last safepoint.
...
- **The core (root module) depends on the Go standard library only — with two accepted, ADR-justified third-party exceptions**, each verified/required to add **zero net-new transitive dependencies**:
  ...
  **`github.com/expr-lang/expr` is no longer a core dependency.** It was one ([ADR 0019](docs/adrs/0019-runtime-expression-evaluation.md)) while the expression-backed endpoints lived in the pattern core; the Plan 027 restructure removed them and **dropped `expr-lang/expr` from the root `go.mod` in this window**. Runtime expression evaluation will live in a separate `expr` provider module, which owns that dependency so no consumer of the core pays for it. **DECIDED but NOT YET IMPLEMENTED — the two orphaned sentinels (`ErrInvalidExpression`, `ErrExprResultType`) are to leave root as well** (decision D-I, 2026-07-28): they still exist in `errors.go` today; Plan 027 Task 9.5 deletes them and Task 10 has the `expr` module declare its own (`msgin/expr:` prefix), matching how every shipped adapter mints sentinels for its own faults. The goal state is that root has a producer, inside the root module, for every sentinel it declares. Related: **D-K** — the provider wraps `ErrExprResultType` in `msgin.Permanent`, since a result-type mismatch is deterministic and must not be retried. See [ADR 0029 §5.0a/§5.0b](docs/adrs/0029-eip-lexical-alignment.md) and [Spec 014 §3.2/§7](docs/specs/014-core-package-layout.md).
...
# workspace coherence (the separate CI `workspace` job) — go.work auto-discovered
go build ./... && (cd adapter/database/sql/postgres && go build ./...)
```
