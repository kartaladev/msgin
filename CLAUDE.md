...
**Active, pre-v1.** The module exists and is substantially implemented: the pattern core at the repo root plus `adapter/{memory,cron,http,http/stdlib,database/sql}`, delivered across **26 plans** (`docs/plans/001`–`026`) / **25 ADRs** (`docs/adrs/0001`–`0026` — 26 numbers but 25 files: **there is no ADR 0024**, the `gin` dependency ADR is a forward reference to an unwritten artifact) / **13 specs** (`docs/specs/001`–`013`). The in-flight Plan 027 restructure adds Spec 014, ADRs 0027–0030 and Plan 027 on top, so `ls docs/adrs/*.md | wc -l` counts **29** today (0001–0030, minus 0024). **Nothing is tagged or released** — there are no consumers, so breaking API changes are still free. Read [`docs/HANDOVER.md`](docs/HANDOVER.md) first: it names the active increment and the last safepoint.
...
- **Multi-instance / distributed-deployment awareness (mandatory when designing any EIP component).** A consumer runs this library in an **unknown deployment topology** — most commonly **N horizontally-scaled app instances behind a load balancer/proxy**, not a single process. When designing any component whose correctness depends on state that spans a message's or request's lifetime — **request-reply correlation, coordination/leader-election, competing consumers/consumer groups, dedup/idempotency, in-memory registries or caches, sequencing/resequencing** — you MUST explicitly reason about the multi-instance case, because **state that lives in one process's memory (a Go channel, an in-memory map, a local `sync.Mutex`) cannot be assumed reachable by another instance**, and a request handled by instance A may have its reply/continuation arrive at instance B. For every such component: (1) **state the topology it targets** — in-process-only, or distributed — in its spec/ADR and godoc; (2) where it is **in-process-only**, say so explicitly and **name the EIP pattern a distributed deployment would require** (e.g. **Return Address** for cross-instance request-reply, a durable/DB lock or lease for coordination, broker **consumer groups** for competing consumers, an **idempotency key / dedup store** for exactly-once, a durable store for resequencing) so the **SPI seam is shaped to accept that pattern without a core change**; (3) **never let an in-process construct silently leak into a contract a distributed deployment would break** — if the core cannot solve the cross-process case (a Go channel cannot cross a process), push it to the adapter/SPI boundary and record the constraint for that future increment (worked example: [Spec 010 §8.1](docs/specs/010-messaging-gateway.md) / [ADR 0022](docs/adrs/0022-messaging-gateway.md) — the gateway correlator is single-process by construction; the future external `RequestReplyExchange` adapter must implement Return Address). Fold this into the design-time adversarial audit: a component that assumes single-process without saying so is an audit finding.
...
- **The core (root module) depends on the Go standard library only — with two accepted, ADR-justified third-party exceptions**, each verified/required to add **zero net-new transitive dependencies**:
  ...
  **`github.com/expr-lang/expr` is no longer a core dependency.** It was one ([ADR 0019](docs/adrs/0019-runtime-expression-evaluation.md)) while the expression-backed endpoints lived in the pattern core; the Plan 027 restructure removed them and **dropped `expr-lang/expr` from the root `go.mod` in this window**. Runtime expression evaluation will live in a separate `expr` provider module, which owns that dependency so no consumer of the core pays for it. **DONE — the two orphaned sentinels (`ErrInvalidExpression`, `ErrExprResultType`) have LEFT root** (decision D-I, 2026-07-28; landed in Plan 027 Task 9.5). Both `var` blocks and their godoc were deleted from `errors.go`, taking root from **43 → 41** sentinels and **102 → 100** exported symbols (`apidiff` removals 95 → 97); `grep -rn 'ErrInvalidExpression\|ErrExprResultType' --include='*.go' .` is now empty workspace-wide. **Still outstanding:** Task 10 has the `expr` module declare **`ErrInvalidExpression`** itself (`msgin/expr:` prefix), matching how every shipped adapter mints sentinels for its own faults. The goal state — root has a producer, inside the root module, for every sentinel it declares — holds as of Task 9.5. Related: **D-K, REVISED 2026-07-28 (audit round 6)** — the `expr` module does **not** declare an `ErrExprResultType` at all. A result-type mismatch is returned as `fmt.Errorf("%w: expr result %T is not %T", msgin.ErrPayloadType, got, want)`, reusing root's existing sentinel so every future expression provider (CEL, starlark, …) shares one `errors.Is` target; no `msgin.Permanent` wrap is needed, because `ErrPayloadType` is already inside `IsPermanent` (`reliability.go:46`). **This supersedes the earlier D-K**, which had the provider mint its own `ErrExprResultType` and wrap it in `msgin.Permanent`. See [ADR 0029 §5.0a/§5.0b](docs/adrs/0029-eip-lexical-alignment.md), [Spec 014 §3.2/§7](docs/specs/014-core-package-layout.md), and the audit record [`docs/plans/027-audit-round-6.md`](docs/plans/027-audit-round-6.md) §1.
...
# exactly this shape, but over its own 6 directories; the crontest entry is the local extra.
for d in . adapter/database/sql/{harness,postgres,mysql,sqlite,dbtest} adapter/cron/crontest; do
