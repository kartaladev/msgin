...
| Plans — **files** | `ls docs/plans/[0-9]*.md \| wc -l` | **58** |
...
2. **For plans, count distinct numbers, not files.** Files exceed numbers because six plans carry satellite audit-round, review-findings and derivation records — 027 (8 files), 028 (5), 029 (6), 030 (2), **031 (6)**, 032 (5). Derive the breakdown with `ls docs/plans/[0-9]*.md | sed -E 's|.*/([0-9]{3}).*|\1|' | sort | uniq -c | awk '$1>1'`. **The file count moves fastest of the five** — every audit round *and every review round* adds a file without adding a number, so assume the number beside it is already stale. It has been wrong twice: Plan 031 alone took 031 from 4 satellites to 6 (four audit rounds, then `031-review-findings.md`), and the file total from 56 to 58.
...
- **Docs links resolve — repo-wide, as a PRE-MERGE gate.** Traceability (see "Documentation artifacts") is only real if the links work, and the recurring break is mechanical: a `docs/plans/*` or `docs/specs/*` file citing an ADR by **bare filename** (`[0003](0003-multi-module-repository-layout.md)`), which resolves relative to the *citing* file's directory and silently 404s. Fix the **class**, not the instance — run this over every tracked Markdown file before merging, and treat any output as a blocker:
  ...
  **Known limitation — read before trusting a hit.** This is `grep`, not a Markdown parser. It strips fenced code blocks and single-line inline code spans **only**, so an inline code span that *wraps across a line* (a Go signature like `Foo[A](x string)` split over two lines) or a code fence nested inside a blockquote (a fence line prefixed with a `>` marker) leaks Go generics into arm 1 as false positives of the shape `-> docs/plans/m`. Two such exist today; both are code, not links. **A hit that names a plausible `.md` path is real; a hit that names a Go identifier or contains a space is the parser limitation.** Arm 2 is exact and every anchor link resolves — **do not cite a count here; it rots on every docs commit** (this line read `53` while the tree held `56`, and the sweep that found the drift added none of them). **Verify arm 2 is not vacuous by planting a bad anchor and re-running** — that is the part with value. *(Done at Plan 028 Task 8: planting `[probe](../CLAUDE.md#this-anchor-does-not-exist-probe)` in `docs/HANDOVER.md` produced exactly one `ANCHOR` hit, which disappeared on revert.)*
