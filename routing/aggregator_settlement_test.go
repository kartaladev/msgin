...
func TestAggregator_NStress(t *testing.T) {
	...
	agg, err := routing.NewAggregator[string, []string](store,
		...
		routing.WithCompletionSize(k),
		routing.WithGroupTimeout(20*time.Millisecond),
...
func TestAggregator_SplitterRoundTrip(t *testing.T) {
	...
	split := routing.Split(func(_ context.Context, m msgin.Message[[]int]) ([]msgin.Message[int], error) {
...
func TestAggregator_ConcurrentClaim_SingleEmitNoLoss(t *testing.T) {
	...
	agg1, err := routing.NewAggregator[string, []string](store, aggFn, opts...)
	require.NoError(t, err)
	agg2, err := routing.NewAggregator[string, []string](store, aggFn, opts...)
	require.NoError(t, err)
	...
	for i := 0; i < m; i++ {
		...
		go func(id string, agg *routing.Aggregator) {
			defer wg.Done()
			msg := msgin.New[any](id, msgin.WithID(id), msgin.WithHeaders(map[string]any{
				msgin.HeaderCorrelationID: "stress-group",
			}))
