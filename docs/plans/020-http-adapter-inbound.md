...
**Tech Stack:** Go 1.25, stdlib only (`net/http`, `net/http/httptest`, `context`, `io`, `log/slog`, `strings`). The `msghttp` and `stdlib` packages are part of the **root module** (like `adapter/memory`, `adapter/cron`) — **no new `go.mod`, no `go.work` change, no new dependency.** (The separate gin module + its dependency would land in Phase 5, under a plan number **assigned when that increment is written**, with **ADR 0024** — number reserved, artifact unwritten, and the gin admission still an open dependency decision. — *corrected 2026-08-22, [Plan 030](030-post-029-maintenance.md) Task 3: this read "**Plan 028**", which [nil option elements](028-nil-option-elements.md) has since consumed; before that, corrected 2026-07-28 audit round 6 M-4 from "Plan 025", which is [`025-http-sse-server.md`](025-http-sse-server.md). No third number is substituted — it would go stale identically.* See [Spec 011 §8](../specs/011-http-adapter.md).)
...
- `DecodeRequest`:
  ...
  3. `msg := msgin.New(body, msgin.WithHeaders(headerMap))` — external input entering the system ⇒ `New` (fresh `HeaderMessageID` + timestamp), not `NewMessage`.
