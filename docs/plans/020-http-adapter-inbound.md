...
**Tech Stack:** Go 1.25, stdlib only (`net/http`, `net/http/httptest`, `context`, `io`, `log/slog`, `strings`). The `msghttp` and `stdlib` packages are part of the **root module** (like `adapter/memory`, `adapter/cron`) — **no new `go.mod`, no `go.work` change, no new dependency.** (The separate gin module + its dependency land in Phase 5 / **Plan 028** / ADR 0024 — corrected 2026-07-28, audit round 6 M-4; this read "Plan 025", which is [`025-http-sse-server.md`](025-http-sse-server.md). See [Spec 011 §8](../specs/011-http-adapter.md).)
