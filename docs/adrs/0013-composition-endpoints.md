...
- **Status:** **Accepted (2026-07-18) — implemented in [Plan 008](../plans/008-composition-endpoints-phase1.md), merged as `a120cd7`.** *(Status corrected 2026-07-28, audit round 6 finding M-3; the line read "Proposed (2026-07-18) … Pending round-2 re-audit + user go-ahead" long after the endpoints shipped. Evidence in non-test code today — relocated to their EIP packages by Plan 027's `c83dde9`: `transform/transformer.go:15` `Transform`, `routing/filter.go:27` `Filter`, `routing/router.go:37` `NewRouter`, `endpoint/activator.go:15` `Activate`.)* Pre-implementation record, retained: **adversarial audit round 1 folded** (`.superpowers/sdd/plan-008-audit-round-1.md`: F1 reuse existing `ErrPayloadType`; F2 `To(OutboundAdapter)`; F3 live-value vs wire scoping; F4 nil-func guards; F5 `WithPayload` header propagation; F6 `Chain`-terminal contract; F8 Activate/Consume split).
